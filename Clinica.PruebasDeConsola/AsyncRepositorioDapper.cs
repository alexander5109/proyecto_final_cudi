...
public class AsyncRepositorioDapper {
    public static async Task<List<Result<Paciente2025>>> GetPacientes() {
        ...
        List<Result<Paciente2025>> resultados = pacientes.Select(x => x.ToDomain().PrintAndContinue("Paciente domainizado")).ToList();
        return resultados;
	}
	public static async Task<List<Medico2025>> GetMedicos() {
  ...
  //      IEnumerable<MedicoDto> medicos = await repo.GetAllMedicos();

  //      List<Result<Medico2025>> resultados = medicos.Select(x => x.ToDomain().PrintAndContinue("Medico domainizado")).ToList();
  //      return resultados;
    }
