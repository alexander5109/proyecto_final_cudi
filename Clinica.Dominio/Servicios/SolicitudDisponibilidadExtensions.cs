                ...
                foreach (var medico in medicos) {
                    ...
                    foreach (var horario in horarios) {
                        ...
                        while (cursor.Add(duracion) <= horario.Hasta.Valor) {
                            ...
                            var ocupado = turnos.Any(t => t.MedicoAsignado is not null && t.MedicoAsignado.Dni.Valor == medico.Dni.Valor && t.FechaYHora < slotEnd && t.FechaYHora.Add(TimeSpan.FromMinutes(t.Especialidad.DuracionConsultaMinutos)) > slotStart);
                            if (!ocupado) {
